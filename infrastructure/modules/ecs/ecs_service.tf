...
resource "aws_ecs_service" "blue_weatherapp_ecsservice" {
  ...
  cluster         = aws_ecs_cluster.blue_weatherapp_cluster.id          # Referencing our created Cluster
  ...
  network_configuration {
    subnets          = var.blue_private_subnets
...
resource "aws_ecs_service" "green_weatherapp_ecsservice" {
  ...
  cluster         = aws_ecs_cluster.green_weatherapp_cluster.id          # Referencing our created Cluster
  ...
  # Define where do we want to launch our containers/tasks
  network_configuration {
    subnets          = var.green_private_subnets
    assign_public_ip = false                                               # Providing our containers with public IPs
